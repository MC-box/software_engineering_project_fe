<template>
    ...
    <h1>添加课程</h1>
    <label>请输入课程名字：</label>
    <a-input v-model:value="coursename" placeholder="请输入标题" style="margin-bottom: 30px;"></a-input>
    <label>请输入课程描述：</label>
    <a-textarea
    ...
    />
    <a-button type="primary" style="float: right" @click="createCourse()">创建课程</a-button>
    <a-divider />
    <!-- 删除课程 -->
    <h1>删除课程</h1>
    <a-select style="width: 250px; margin-right: 30px;" :value="value1" @focus="focus" @change="handleChange">
        <a-select-option v-for="deleteinfo in deletecourseinfos" :key="deleteinfo.courseid" >{{ '课程名：' + deleteinfo.name + ' 课程id：' + deleteinfo.courseid}}</a-select-option>
    </a-select>

    <a-button type="primary" @click="deleteCourse()" :disable="value1 === disabled">删除该课程</a-button> 
    <!-- 无法使用disable -->
